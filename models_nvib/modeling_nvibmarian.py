...
class DenoisingAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            nvib_dict["log_alpha"] = torch.cat(
                [
                    nvib_dict["log_alpha"][:, 0:1, :],
                    past_alpha,
                    nvib_dict["log_alpha"][:, 1:, :],
                ],
                dim=1,
            )
...
class MarianModelNVIB(MarianPreTrainedModel):
...
class MarianMTModelNVIB(MarianPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: MarianConfig):
        super().__init__(config)
        self.model = MarianModelNVIB(config)
    ...
    def _resize_token_embeddings(
        self, new_num_tokens: int, pad_to_multiple_of=None
    ) -> nn.Embedding:
